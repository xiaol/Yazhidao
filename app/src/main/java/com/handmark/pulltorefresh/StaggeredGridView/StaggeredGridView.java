...
package com.handmark.pulltorefresh.StaggeredGridView;
 ...
 * <p/>
 * <p>Item views may span multiple columns as specified by their {@link com.handmark.pulltorefresh.StaggeredGridView.StaggeredGridView.LayoutParams}.
...
public class StaggeredGridView extends ViewGroup {
    ...
    private int m_pContentOffsetY=0;
...
//                }
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            mLastTouchY = y;
            final boolean stopped = !trackMotionScroll(dy, true);
...
//        drawSelector(canvas);
    ...
    private void populate(boolean clearData) {
        ...
        for (int i = 0; i < colCount; i++) {
            ...
            Log.e("----","mItemBottoms="+mItemBottoms[i]+" i="+i);
...
//        	displayMapping();
            ...
            if (lp == null) {
                Log.e("----","child.getTop()="+child.getTop());
...
//                nextCol = rec.column;
            ...
            if (span > 1) {
                ...
                int lowestView = 0;
                for (int i = 0; i < mColCount; i++) {
                    if (mItemBottoms[i] > lowestView&&i%mColCount==nextCol)
                    {
                        lowestView = mItemBottoms[i];
                    }
                }

                int highestView = lowestView;
                Log.e("----","i%mColCount="+position+"getChildCount()="+getChildCount());

                for (int i = getChildCount()-1; i>=1 ; i--) {
                    final View child1 = getChildAt(i);
                    if (child1 == null) {
//                highestView = 0;
                        break;
                    }
                    ...
                    final int top = child1.getTop();
                    final int left = child1.getLeft();
                    Log.e("----","i%mColCount="+(left-paddingLeft)*1.0f/(colWidth + itemMargin)+" nextCol="+nextCol+" top="+top+" highestView="+highestView);
                    if (top < highestView)
                    {
                        if ((left-paddingLeft)*1.0f/(colWidth + itemMargin)==nextCol){
                            highestView = top;
                        }
                    }
                }
                startFrom = highestView-mItemMargin;
//                for (int j=0;j<mItemTops.length;j++)
//                {
//                    Log.e("----","mItemTops"+mItemTops[j]+"j="+j);
//                }
...
//                final int colEnd = Math.min(mColCount, nextCol + lp.span);
                ...
                startFrom = lowestView;
//                for (int j =0;j<mItemBottoms.length;j++)
//                {
//                    Log.e("----","j="+j+" value="+mItemBottoms[j]);
//                }
//                startFrom = mItemBottoms[nextCol];
            ...
            final int colEnd = Math.min(mColCount, nextCol + lp.span);
            for (int i = nextCol; i < colEnd; i++) {
               Log.e("----","childBottom + rec.getMarginBelow(i - nextCol)="+(childBottom + rec.getMarginBelow(i - nextCol))+" childBottom="+childBottom +" rec.getMarginBelow(i - nextCol)="+ rec.getMarginBelow(i - nextCol));
                    ...
                    if (childCount > 0) {
                        ...
                        for (int i = 0; i < childCount; i++) {
                            ...
                            for (int col = lp.column; col < colEnd; col++) {
                                final int colTop = top;
                                final int colBottom = bottom;
                                if (colTop < mItemTops[col]&&mItemTops[col]>=0) {
                                    mItemTops[col] = colTop;
                                }
                                if (colBottom > mItemBottoms[col]) {
                                    mItemBottoms[col] = colBottom;
